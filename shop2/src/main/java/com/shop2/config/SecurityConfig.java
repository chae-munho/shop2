...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
                ...
                //애플리케이션 기동 후 첫 post에서 csrf 토큰이 생성되지 않는 문제로 인해서 CsrfCookieFilter 추가 즉 csrf 토큰이 없는 경우 강제로 생성
                //CsrfCookieFilter는 첫 요청에서 CSRF 토큰이 없을 경우 자동으로 생성하여 응답의 쿠키에 추가하는 역할을 합니다.
